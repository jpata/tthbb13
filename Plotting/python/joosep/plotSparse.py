...
def category_report(path, category):
    ...
    plotlib.svfg("output/plots/{0}.pdf".format(category.discriminator))
    plotlib.svfg("output/plots/{0}.png".format(category.discriminator))
    plt.clf()
    del fig

    systout_fn = "output/{0}_{1}.html".format(category.discriminator, category.name)
    ...
    for s1, s2 in syst_pairs:
        ...
        for sample in r[2].keys():
            ...
            if not os.path.exists(p):
                os.makedirs(p)
            plotlib.svfg("output/plots/{0}/syst{1}/{2}_{3}.pdf".format(category.discriminator, syst_name, category.name, sample))
            plotlib.svfg("output/plots/{0}/syst{1}/{2}_{3}.png".format(category.discriminator, syst_name, category.name, sample))
            systout.write(
                '<a href="plots/{0}/syst{1}/{2}_{3}.pdf"><img width="300" src="plots/{0}/syst{1}/{2}_{3}.png"></a>\n'.format(
...
category_report("/Users/joosep/Documents/tth/data/histograms/GCa5653586a59e/AnalysisSpecification/", analysis.categories[0])
# 
