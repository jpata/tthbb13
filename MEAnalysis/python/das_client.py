...
class HTTPSClientAuthHandler(urllib2.HTTPSHandler):
    ...
    key/ca information
    """
    def __init__(self, cert=None, cert_dir=None, level=0):
        if  level > 1:
    ...
    def get_connection(self, host, timeout=300):
        ...
        if self.cert:
            ...
            context.load_verify_locations(capath=self.cert_dir)
...
class DASOptionParser: 
    ...
    def __init__(self):
        ...
        self.parser.add_option("--cert", action="store", type="string",
                               default=os.environ["X509_USER_PROXY"], dest="cert", help=msg)
        msg  = 'CA path'
        self.parser.add_option("--cert_path", action="store", type="string",
                               default=os.environ["X509_CERT_DIR"], dest="cert_path", help=msg)
...
def get_data(host, query, idx, limit, debug, threshold=300,
        cert=None, cert_path=None, das_headers=True):
    ...
    if cert and cert_path:
        cert = fullpath(cert)
        http_hdlr  = HTTPSClientAuthHandler(cert, cert_path, debug)
    else:
...
def main():
    ...
    if  opts.format == 'plain':
        jsondict = get_data(host, query, idx, limit, debug, thr, cert, cert_path)
        ...
        if  jsondict['status'] != 'ok':
            ...
            if  opts.retry:
                ...
                for attempt in xrange(1, int(opts.retry)):
                    ...
                    time.sleep(interval)
                    data = get_data(host, query, idx, limit, debug, thr, cert, cert_path)
                    jsondict = json.loads(data)
        ...
        jsondict = get_data(\
                host, query, idx, limit, debug, thr, cert, cert_path)
        print(json.dumps(jsondict))
