...
TTHNtupleAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (const pat::Electron &x : *electrons) {
    ...
    for (const auto& ele_id : eleIdentifiers_) {
      ...
      if (x.electronID(ele_id)) {
        //passes, flip bit with index lep_id_idx to ON
        tthtree->lep__id_bitmask[n__lep] = tthtree->lep__id_bitmask[n__lep] | (1 << ele_id_idx);
      } else {
        //fails, flip bit with index lep_id_idx to OFF
        tthtree->lep__id_bitmask[n__lep] = tthtree->lep__id_bitmask[n__lep] & ~(1 << ele_id_idx);
      }
      ele_id_idx += 1;
      if (ele_id_idx>= 32) {
  ...
  for (const pat::Tau &x : *taus) {
    ...
    tthtree->tau__eta[n__tau] = x.eta();
    tthtree->tau__pt[n__tau] = x.pt();
    tthtree->tau__phi[n__tau] = x.phi();
    tthtree->tau__mass[n__tau] = x.mass();
    tthtree->tau__id[n__tau] = x.pdgId();
    tthtree->tau__charge[n__tau] = x.charge();

    //tthtree->tau__dxy[n__tau] = x.dxy();

    //tthtree->tau__ch_iso[n__tau] = x.chargedHadronIso();
    //tthtree->tau__puch_iso[n__tau] = x.puChargedHadronIso();
    //tthtree->tau__ec_iso[n__tau] = x.ecalIso();
    //tthtree->tau__hc_iso[n__tau] = x.hcalIso();
    //tthtree->tau__p_iso[n__tau] = x.particleIso();
    //tthtree->tau__ph_iso[n__tau] = x.photonIso();
    
    //tthtree->tau__is_tight[n__tau] = is_good_tau(x);
    //tthtree->tau__is_loose[n__tau] = is_good_tau(x); //FIXME: implement loose tau ID
    if (x.leadChargedHadrCand().isNonnull()) {
      tthtree->tau__lead_ch_had__pt[n__tau] = x.leadChargedHadrCand()->pt();
    }
    tthtree->tau__pf_jet__pt[n__tau] = x.pfEssential().p4Jet_.pt();
    tthtree->tau__pf_jet__eta[n__tau] = x.pfEssential().p4Jet_.eta();
    tthtree->tau__pf_jet__phi[n__tau] = x.pfEssential().p4Jet_.phi();
    tthtree->tau__pf_jet__mass[n__tau] = x.pfEssential().p4Jet_.mass();

    tthtree->tau__pf_cjet__pt[n__tau] = x.pfEssential().p4CorrJet_.pt();
    tthtree->tau__pf_cjet__eta[n__tau] = x.pfEssential().p4CorrJet_.pt();
    tthtree->tau__pf_cjet__phi[n__tau] = x.pfEssential().p4CorrJet_.pt();
    tthtree->tau__pf_cjet__mass[n__tau] = x.pfEssential().p4CorrJet_.pt();

	if (x.pfEssential().sv_.isNonnull()) {
    	tthtree->tau__vtx__z[n__tau] = x.pfEssential().sv_->z();
	}    
	tthtree->tau__id__ch_iso[n__tau] = x.tauID("chargedIsoPtSum");
    tthtree->tau__id__n_iso[n__tau] = x.tauID("neutralIsoPtSum");
    tthtree->tau__id__puc[n__tau] = x.tauID("puCorrPtSum");
    tthtree->tau__dmode[n__tau] = x.decayMode();
    
    if (x.genLepton() != 0) {
      tthtree->gen_tau_lep__pt[n__tau] = x.genLepton()->pt();
      tthtree->gen_tau_lep__eta[n__tau] = x.genLepton()->eta();
      tthtree->gen_tau_lep__phi[n__tau] = x.genLepton()->phi();
      tthtree->gen_tau_lep__mass[n__tau] = x.genLepton()->mass();
      tthtree->gen_tau_lep__id[n__tau] = x.genLepton()->pdgId();
    }


    if (x.genJet() != 0) {
      tthtree->gen_tau_jet__pt[n__tau] = x.genJet()->pt();
      tthtree->gen_tau_jet__eta[n__tau] = x.genJet()->eta();
      tthtree->gen_tau_jet__phi[n__tau] = x.genJet()->phi();
      tthtree->gen_tau_jet__mass[n__tau] = x.genJet()->mass();
      tthtree->gen_tau_jet__id[n__tau] = x.genJet()->pdgId();
    }
    ...
    tthtree->tau__id_bitmask[n__tau] = 0;
    for (const auto& tau_id : tauIdentifiers_) {
      if (x.tauID(tau_id) > 0.5) {
        //passes, flip bit with index tau_id_idx to ON
        tthtree->tau__id_bitmask[n__tau] = tthtree->tau__id_bitmask[n__tau] | (1 << tau_id_idx);
      } else {
